...
// 载入数据
func loadData(sDbFile string) *Assh {
	data := make(map[string]map[string]*Server)

	if kiris.FileExists(sDbFile) {
		c, err := kiris.FileGetContents(sDbFile)
		if err != nil {
			log.Fatal(err)
		}
		...
		servers := Assh{data: data, dbFile: sDbFile}
		return &servers
	}

	return &Assh{data: data, dbFile: sDbFile}
}
