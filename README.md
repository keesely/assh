...
| ☁️ **云同步** | 七牛云加密同步服务器配置，跨设备共享 |
| ❤️ **健康检查** | 连接延迟 + 系统指标采集，单机/批量/分组巡检 |
...
```bash
# 添加新服务器
assh add myserver -H 192.168.1.100 -u root -P mypassword -p 22
...
# 带备注添加
assh add dev.node -H 10.0.0.2 -u dev -P devpass --remark "开发环境"
```
...
```bash
# 添加服务器
assh add <name> -H <host> [-p port] [-u user] [-P password] [-i keyfile] [--group group] [--remark remark]

# 创建或更新服务器（upsert）
assh set <name> [-H host] [-p port] [-u user] [-P password] [-i keyfile] [-o option] [--remark remark]
assh set <name> --clear-password   # 清除密码
assh set <name> --clear-key        # 清除密钥文件

# 列出服务器
assh ls                    # 全部列表
assh ls --group prod       # 按分组筛选
assh ls --search example   # 关键词搜索
...
| `add` | 添加服务器 | ✅ Phase 4 |
| `set` | 创建/更新服务器（upsert） | ✅ Phase 4 |
| `ls` | 列出服务器 | ✅ Phase 4 |
| `info` | 查看服务器详情（含模糊匹配建议） | ✅ Phase 4 |
| `rm` | 删除服务器 | ✅ Phase 4 |
| `mv` | 重命名/移动服务器 | ✅ Phase 4 |
| `rollback` | 回滚服务器配置 | ✅ Phase 4 |
...
| `sync` | 云同步（push/pull/account/history） | ✅ Phase 8 |
| `health` | 服务器健康检查（check/list） | ✅ Phase 9 |
...
| 8 | 云同步 (Qiniu) | ✅ 完成 | 2026-05-16 |
| 9 | 服务器健康检查 | ✅ 完成 | 2026-05-17 |
| 10 | 跳板机/堡垒机支持 | ⏳ 待开始 | — |
...
| 12 | 自动更新/升级 | ⏳ 待开始 | — |
| 13 | 远程命令执行增强 (超时/输出格式化/变量替换) | ⏳ 待开始 | — |
| X1 | 实验：AI 安全检查 | 🆕 进行中 (2026-05-17) | — |
| X2 | 实验：虚拟命令投放与执行 | ⏳ 待开始 | — |

> 📖 **完整使用手册**：[docs/USAGE.md](../docs/USAGE.md) — 涵盖所有功能的中文示例驱动文档，含 150+ 场景示例和最佳实践
...
│   ├── keygen.go            # 密钥生成命令
│   ├── sync.go              # 云同步命令（account/push/pull/history）
│   ├── health.go            # 健康检查命令（check/list）
...
│   │   ├── syncer.go        # Syncer 接口（云同步）
│   │   ├── health.go        # HealthChecker 接口
